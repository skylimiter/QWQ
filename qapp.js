...
function nav(path) {
    ...
    let search_text = model.is_search_page ? model.q || '' : '',
        ...
        <a href="//discord.com/invite/XrAa7RDBhw" target="_blank" class="mdui-btn mdui-btn-icon mdui-ripple mdui-ripple-white" mdui-tooltip="{content: \'點我加入群組！\'}" alt="點我加入群組！"><img src="https://img-premium.flaticon.com/png/512/356/356009.png?token=exp=1623328924~hmac=ee658b20ee0e156b0e4b04ffbb8dfe59" width="50" height="45"></a>
...
function file_mkv(path) {
    ...
    playBtn += `<br><a style="margin-top: 15px" href="${encoded_url}" class="${btnClass} download-btn">直連下載</a>`
    ...
    ${playBtn}
    <div class="mdui-textfield">
        <label class="mdui-textfield-label mdui-text-color-white">偵測到檔案為 MKV ，本格式不支援線上觀看，請使用串流或下載。</label>
    </div>
    <hr>
    </div>`
    $('#content').html(content)
...
function file_video(path) {
    ...
    // 直連下載
    playBtn += `<br><a style="margin-top: 15px" href="${encoded_url}" class="${btnClass2} download-btn">直連下載</a>`
    ...
    ${switchElement}
    <div class="mdui-textfield">
        <label class="mdui-textfield-label mdui-text-color-white">注意：若影片無畫面，請重新整理或使用播放器串流。</label>
    </div>
    <hr>
    </div>`
    $('#content').html(content)
